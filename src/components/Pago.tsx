...
interface Props {
  ...
  date:string;
  currency:string;
  handlerPercentageBus:(id:number, action:string)=>void;
}
function Pago({editable, id, name, value, percentage, date, currency, handlerPercentageBus}:Props) {
  ...
  return (
    <div className="w-fit h-fit grid gap-y-2 z-20">
      <div className="bg-gray-50 w-12 aspect-square mx-auto border-[3px] border-[#FC4024] rounded-full"></div>
      <input 
        type="text"
        disabled={!editable}
        id="inputName"
      ...
      <div className={`flex mx-auto pl-2 ${editable?"w-full border-[0.5px]":"w-fit gap-x-1"} border-gray-400`}>
        ...
        <p className={`text-sm font-semibold ${editable?"text-gray-400 translate-x-[160%] transition duration-500":"text-gray-900"}`}>{currency}<span hidden={editable} className="font-normal"> ({inputPercentage}%)</span></p>
      </div>
      <div className={`mx-auto gap-x-2 ${editable?"flex":"hidden"}`}>
        <button onClick={()=>{handlerPercentageBus(id,"-")}} className="bg-gray-50 w-6 aspect-square rounded-full border-[0.58px] border-[#FF7A66] hover:border-[#FC4024] hover:scale-110 transition duration-200 grid">
